...
WriteVmConfig::WriteVmConfig(Task* clone_leader, const char* data_dir,
  ...
  buffer = { .ptr = (uint8_t*)::mmap(nullptr, buffer_size, PROT_READ | PROT_WRITE,
...
ssize_t WriteVmConfig::pread(ssize_t bytes_read, const KernelMapping& km) const {
  DEBUG_ASSERT(bytes_read != 1 && "you've passed in the 'invalid pread result' as bytes_read");
  const auto current_read = ::pread(proc_mem_fd, buffer.ptr + bytes_read, km.size() - bytes_read, km.start().as_int() + bytes_read);
  if(current_read == -1)
    return current_read;
...
void update_persistent_checkpoint_index(
  ...
  for (const auto& cp : checkpoints) {
    ...
    tuid.setGroupId(cp.last_continue_task.tguid.tid());
    tuid.setGroupSerial(cp.last_continue_task.tguid.serial());
    tuid.setTaskId(cp.last_continue_task.tuid.tid());
    tuid.setTaskSerial(cp.last_continue_task.tuid.serial());

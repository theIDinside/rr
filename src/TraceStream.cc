...
void TraceReader::forward_to(FrameTime next_event_to_start_at) {
  while (!at_end()) {
    auto frame = read_frame();
    // means the EVENTS stream is at the correct time, now RAW_DATA and MMAPS must catch up 1 "step"
    if (next_event_to_start_at-1 == frame.time()) {
      auto& mmaps = reader(MMAPS);
      while(!mmaps.at_end()) {
    ...
    if (frame.time() < next_event_to_start_at) {
      ...
      while (true) {
        TraceReader::MappedData data;
        KernelMapping km = read_mapped_region(&data, nullptr, TraceReader::DONT_VALIDATE, TimeConstraint::CURRENT_TIME_ONLY);
        if (km.size() == 0) {
          break;
        }
      }

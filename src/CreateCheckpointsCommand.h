#ifndef RR_CREATE_CHECKPOINTS_COMMAND_H_
#define RR_CREATE_CHECKPOINTS_COMMAND_H_
...
protected:
  CreateCheckpointsCommand(const char* name, const char* help) : Command(name, help) {}

  static CreateCheckpointsCommand singleton;

private:
  /* Runs the actual replay, creating checkpoints at events `frames_to_checkpoint_at`. */
  int run_main(const std::string& trace_dir, const std::vector<FrameTime>& frames_to_checkpoint_at);
  ...
  static std::vector<FrameTime> find_events_to_checkpoint(const std::string& trace_dir, const CreateCheckpointsFlags& interval);
  bool verify_params_ok(const CreateCheckpointsFlags& cp);
};

}

#endif // RR_CREATE_CHECKPOINTS_COMMAND_H_

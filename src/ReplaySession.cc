...
void ReplaySession::serialize_checkpoint(CheckpointInfo& cp_info) {
  ...
  address_spaces.reserve(as_data.size());
  for(const auto& as_clone : as_data) {
    address_spaces.push_back(as_clone.clone_leader->vm().get());
  }
...
void ReplaySession::load_checkpoint(
  ...
  for (const auto& as : addr_spaces) {
    ...
    leader->post_exec(exe_name, original_exe_name);
    static_cast<Task*>(leader)->post_exec_syscall();
    ...
    // calls
    {
      ...
      leader->vm()->unmap_all_but_rr_page(remote);
